package owogModel;

import java.math.BigDecimal;

public class OwogJkb {

	// Fields
	private String id;
	private BigDecimal zb11; // 指标11
	private BigDecimal zb12; // 指标12
	private BigDecimal zb21; // 指标21
	private BigDecimal zb22; // 指标22
	private BigDecimal zb31; // 指标31
	private BigDecimal zb32; // 指标32
	private BigDecimal zb41; // 指标41
	private BigDecimal zb42; // 指标42
	private BigDecimal ldpf; // 领导评分
	private BigDecimal qzzbsum; // 权重指标合计
	private BigDecimal jf1; // 加分1
	private BigDecimal jf2; // 加分2
	private BigDecimal jf3; // 加分3
	private BigDecimal jf4; // 加分4
	private BigDecimal jf5; // 加分5
	private BigDecimal jf6; // 加分6
	private BigDecimal allsum; // 所有合计
	private String ypfj1; // 一票否决1
	private String ypfj2; // 一票否决2
	private String ypfj3; // 一票否决3
	private String ypfj4; // 一票否决4
	private String ypfj5; // 一票否决5
	private String name; // 姓名
	private String rangetime; // 时间周期
	private BigDecimal ranking; // 排名

	public String getId() {
		return id;
	}

	public void setId(String id) {
		this.id = id;
	}

	public BigDecimal getZb11() {
		return zb11;
	}

	public void setZb11(BigDecimal zb11) {
		this.zb11 = zb11;
	}

	public BigDecimal getZb12() {
		return zb12;
	}

	public void setZb12(BigDecimal zb12) {
		this.zb12 = zb12;
	}

	public BigDecimal getZb21() {
		return zb21;
	}

	public void setZb21(BigDecimal zb21) {
		this.zb21 = zb21;
	}

	public BigDecimal getZb22() {
		return zb22;
	}

	public void setZb22(BigDecimal zb22) {
		this.zb22 = zb22;
	}

	public BigDecimal getZb31() {
		return zb31;
	}

	public void setZb31(BigDecimal zb31) {
		this.zb31 = zb31;
	}

	public BigDecimal getZb32() {
		return zb32;
	}

	public void setZb32(BigDecimal zb32) {
		this.zb32 = zb32;
	}

	public BigDecimal getZb41() {
		return zb41;
	}

	public void setZb41(BigDecimal zb41) {
		this.zb41 = zb41;
	}

	public BigDecimal getZb42() {
		return zb42;
	}

	public void setZb42(BigDecimal zb42) {
		this.zb42 = zb42;
	}

	public BigDecimal getLdpf() {
		return ldpf;
	}

	public void setLdpf(BigDecimal ldpf) {
		this.ldpf = ldpf;
	}

	public BigDecimal getQzzbsum() {
		return qzzbsum;
	}

	public void setQzzbsum(BigDecimal qzzbsum) {
		this.qzzbsum = qzzbsum;
	}

	public BigDecimal getJf1() {
		return jf1;
	}

	public void setJf1(BigDecimal jf1) {
		this.jf1 = jf1;
	}

	public BigDecimal getJf2() {
		return jf2;
	}

	public void setJf2(BigDecimal jf2) {
		this.jf2 = jf2;
	}

	public BigDecimal getJf3() {
		return jf3;
	}

	public void setJf3(BigDecimal jf3) {
		this.jf3 = jf3;
	}

	public BigDecimal getJf4() {
		return jf4;
	}

	public void setJf4(BigDecimal jf4) {
		this.jf4 = jf4;
	}

	public BigDecimal getJf5() {
		return jf5;
	}

	public void setJf5(BigDecimal jf5) {
		this.jf5 = jf5;
	}

	public BigDecimal getJf6() {
		return jf6;
	}

	public void setJf6(BigDecimal jf6) {
		this.jf6 = jf6;
	}

	public BigDecimal getAllsum() {
		return allsum;
	}

	public void setAllsum(BigDecimal allsum) {
		this.allsum = allsum;
	}

	public String getYpfj1() {
		return ypfj1;
	}

	public void setYpfj1(String ypfj1) {
		this.ypfj1 = ypfj1;
	}

	public String getYpfj2() {
		return ypfj2;
	}

	public void setYpfj2(String ypfj2) {
		this.ypfj2 = ypfj2;
	}

	public String getYpfj3() {
		return ypfj3;
	}

	public void setYpfj3(String ypfj3) {
		this.ypfj3 = ypfj3;
	}

	public String getYpfj4() {
		return ypfj4;
	}

	public void setYpfj4(String ypfj4) {
		this.ypfj4 = ypfj4;
	}

	public String getYpfj5() {
		return ypfj5;
	}

	public void setYpfj5(String ypfj5) {
		this.ypfj5 = ypfj5;
	}

	public String getName() {
		return name;
	}

	public void setName(String name) {
		this.name = name;
	}

	public String getRangetime() {
		return rangetime;
	}

	public void setRangetime(String rangetime) {
		this.rangetime = rangetime;
	}

	public BigDecimal getRanking() {
		return ranking;
	}

	public void setRanking(BigDecimal ranking) {
		this.ranking = ranking;
	}

}
